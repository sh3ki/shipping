...
export default function VerifyEmail({ status }: { status?: string }) {
    ...
    return (
        <>
            <Head title="Email verification - CargoNest" />
            <div className="relative flex items-center justify-center text-[#1b1b18] dark:text-[#EDEDEC] font-sans" style={{height: '100vh'}}>
                ...
                <div className="relative w-full max-w-3xl mx-auto flex flex-col md:flex-row bg-white/80 dark:bg-[#18181b]/80 rounded-2xl shadow-2xl overflow-hidden animate-fade-in" style={{backdropFilter: 'blur(4px)', height: '85vh', margin: 'auto'}}>
                    ...
                    <div className="w-full md:w-3/5 md:px-16 md:py-8 flex flex-col justify-center" style={{height: '100%'}}>
                        ...
                        <h2 className="text-xl text-center md:text-2xl font-bold text-[#1b1b18] dark:text-white mb-1">Verify Email</h2>
                        <p className="text-sm text-[#64748b] dark:text-[#cbd5e1] text-center mb-8">Please verify your email address by clicking on the link we just emailed to you.</p>
                        {status === 'verification-link-sent' && (
                            <div className="mb-4 text-center text-sm font-medium text-green-600">
                                A new verification link has been sent to the email address you provided during registration.
                            </div>
                        )}
                        <form className="space-y-4 text-center" onSubmit={submit} autoComplete="off">
                            <button
                                type="submit"
                                disabled={processing}
                            ...
                            <a href={route('logout')} className="mx-auto block text-xs underline underline-offset-4 hover:text-[#2d3748] dark:hover:text-white transition-colors duration-200" style={{marginTop: '1rem'}}>
                                Log out
                            </a>
                        </form>
                    </div>
